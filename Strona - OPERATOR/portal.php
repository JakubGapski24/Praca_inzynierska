...
<html lang="pl">
	...
	//pobrana z ostatniego rekordu bazy data ostatniego łączenia się systemu z serwerem jest porównywana z datą pobraną w dokładnie ten sam sposób metodą AJAX, jeśli się różnią, to znaczy, że wstawiono nowe dane i należy odświeżyć stronę
				...
				var jakas_zmienna = setInterval(function() {
						...
						xmlhttp.open("GET", "nowa_data.php?q=" + stara_data, true);
						xmlhttp.send();
	...
	//********************************************************** Czujnik płomienia *******************************************************************
		...
		$(() => {
		  ...
		  document.getElementById("czujnik_plomienia").style.cursor = 'default'; //zmień kursor myszki z "łapki" na domyślny
	...
	//************************************************************** Czujnik dymu ********************************************************************
		$(() => {
		  ...
		  document.getElementById("czujnik_dymu").style.cursor = 'default'; //zmień kursor myszki z "łapki" na domyślny
	...
	//****************************************************************** ROP *************************************************************************
		$(() => {
		  ...
		  document.getElementById("rop").style.cursor = 'default'; //zmień kursor myszki z "łapki" na domyślny
	...
	//*********************************************************** Czujnik temperatury ****************************************************************
		$(() => {
		  ...
		  document.getElementById("czujnik_temperatury").style.cursor = 'default'; //zmień kursor myszki z "łapki" na domyślny
		...
		class CustomDraggable extends Draggable {
		  ...
		  _onDragStart() { //jeśli tylko zaczniesz przesuwać, to przeładuj stronę - dzięki temu uniemożliwione będzie jej przesuwanie
			this.isActive = true;
			location.reload();
		  }
		  
		  _onPress() { //jeśli tylko klikniesz na czujnik, to przeładuj stronę - dzięki temu uniemożliwione będzie jej przesuwanie
			if (!this.isActive) {
...
<!-- ******************************************************************* MENU ******************************************************************** -->
	<div class="container">
			...
			<div class="menu" id="MENU"> <!-- pełna wersja menu dla usera -->
				<ol>
					<li class="active"> <a href="portal.php"> Strona główna </a></li>
									...
									var wlacz_funkcje = setInterval(function() {
											...
											xmlhttp.onreadystatechange = function() {
												if (this.readyState == 4 && this.status == 200) {
													...
													if (roznica > 70 && czy_online == true)
													{
														location.href="portal.php";
													}
												}
											};
											xmlhttp.open("GET", "czy_online.php?q=" + roznica, true);
											xmlhttp.send();
